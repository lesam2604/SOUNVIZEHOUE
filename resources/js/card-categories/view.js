...
async function deleteObject() {
  try {
    let swalResult = await Swal.fire({
      title: `Voulez-vous vraiment supprimer la catégorie de carte ${object.code}?`,
    ...
    let { data } = await ajax({
      url: `${API_BASEURL}/card-categories/delete/${object.id}`,
      type: 'POST',
    });
...
window.render = async function () {
  ...
  setTitle(`Détails de la catégorie de carte ${object.code}`);

  displayObject();

  $('#edit').click(function (e) {
    location = `/card-categories/${object.id}/edit`;
  });
